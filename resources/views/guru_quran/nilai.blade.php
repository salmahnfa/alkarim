										...
										<table id="nilai-datatables" class="display table table-striped table-hover" >
											...
											<tbody>
												@foreach ($nilais as $nilai)
													<tr data-kelompok-halaqah-id="{{ $nilai->siswa->kelompok_halaqah_id }}" data-ujian-id="{{ $nilai->ujian->id }}">
...
<script>
	$(document).ready(function() {
		...
		$('#displayData').on('click', function() {
			...
			var ujianId = $('#dropdown-ujian').val();
			...
			var filteredRows = tableRows.filter(function() {
				...
				var rowUjianId = $(this).data('ujian-id');
				return rowKelompokHalaqahId == kelompokHalaqahId && rowUjianId == ujianId;
			});
			filteredRows.show();

			if (filteredRows.length === 0) {
				var noDataRow = $('<tr><td colspan="9" class="text-center">Belum ada data yang dimasukkan.</td></tr>');
				$('#nilai-datatables tbody').append(noDataRow);
			} else {
				filteredRows.each(function(index) {
				$(this).find('td:first').text(index + 1);
				});
				$('#nilai-datatables tbody tr:last-child').remove();
			}
		});
	});
</script>
